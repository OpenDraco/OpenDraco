...
async def run_evaluation(req: EvaluationRequest):
    ...
    def worker() -> None:
        try:
            ...
            # `needs_wsl: True` wraps in WSL (swebench is POSIX-only) — but only
            # on Windows. Everywhere else, including the Linux container image and
            # a native Linux/macOS install, the harness is POSIX-native and runs
            # under the active interpreter; shelling out to `wsl` there fails with
            # FileNotFoundError. Mirrors the guard in opendraco/cli.py.
            script_path = BASE_DIR / "scripts" / "evaluation" / f"{forced}.py"
            if platform.system() == "Windows" and manifest.get("needs_wsl"):
                cmd = [
                    "wsl",
