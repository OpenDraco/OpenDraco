...
from opendraco.agents.llm_tool_agent import LLMToolAgent
from opendraco.agents.types import TYPE_REGISTRY
from opendraco.config.loader import load_config, resolve_variant_block
from opendraco.core.workflow.graph_builder import build_graph
from opendraco.core.workflow.state_factory import build_initial_state, build_state_class
from opendraco.exceptions.errors import ConfigError, OpendracoError, OllamaMemoryError
from opendraco.utils.patch import generate_diff_impl
from opendraco.utils.ollama_preflight import preflight_models
from opendraco.utils.workspace import Workspace, clone_workspace

logger = logging.getLogger(__name__)

# Per-node revisit budget. Total super-step budget =
# DEFAULT_MAX_REVISITS * len(agents). Override via OPENDRACO_GRAPH_MAX_REVISITS.
DEFAULT_MAX_REVISITS: int = 2


def _max_revisits() -> int:
    raw = os.getenv("OPENDRACO_GRAPH_MAX_REVISITS", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "OPENDRACO_GRAPH_MAX_REVISITS=%r is not an int; using default %d",
...
def _run_impl(instance: dict[str, Any], config: str | dict[str, Any] = "") -> str:
    """Run the OpenDraco workflow for a single SWE-bench instance. `config` is
    ...
    want SOMETHING the tool sandbox can resolve `OPENDRACO_WORKSPACE_PATH`
    against."""
    ...
    if repo and base_commit:
        ...
        ws_path = Path(tempfile.gettempdir()) / "opendraco_workspace" / f"adhoc-{instance_id}"
    ...
    os.environ["OPENDRACO_WORKSPACE_PATH"] = str(workspace.path)
    ...
    try:
        ...
        raise OpendracoError(f"graph failure for {instance_id}: {exc}") from exc
