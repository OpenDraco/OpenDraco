"""MCP registration + behavior checks for `opendraco.tools.claude_coder`.
...
a few are intentional stubs for runtimes OpenDraco doesn't ship.
"""
...
from opendraco.tools.repo.claude_coder import (
...
def test_attempt_completion_tool_writes_marker(buggy_repo: Path) -> None:
    ...
    assert (buggy_repo / ".opendraco" / "state.json").is_file()
