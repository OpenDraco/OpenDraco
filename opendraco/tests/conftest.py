...
def buggy_repo(tmp_path: Path) -> Generator[Path, None, None]:
    ...
    subprocess.run(["git", "config", "user.email", "test@opendraco.local"], cwd=repo, check=True)
    subprocess.run(["git", "config", "user.name", "opendraco-test"], cwd=repo, check=True)
...
def buggy_instance(buggy_repo: Path) -> dict:
    ...
    return {
        "instance_id": "opendraco__buggy-1",
        "repo": "opendraco/buggy",
