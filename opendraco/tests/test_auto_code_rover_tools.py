"""MCP registration + shape assertions for `opendraco.tools.auto_code_rover`.
...
`opendraco/tests/test_tools.py`: every `@tool`-decorated callable in
...
from opendraco.tools.repo.auto_code_rover import agent_write_patch
