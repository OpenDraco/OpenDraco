"""Tests for `opendraco.utils.ollama_preflight.preflight_models`.
...
`OpendracoError` when a pull can't succeed (no `ollama` binary, registry
...
from opendraco.exceptions.errors import OpendracoError
from opendraco.utils import ollama_preflight
...
def test_raises_when_ollama_binary_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `OpendracoError` with a clear message pointing the user at the
    install URL."""
    cfg = {"agents": {"a": {"model": "ollama/qwen3:8b"}}}
    ...
    with pytest.raises(OpendracoError, match="ollama"):
        ollama_preflight.preflight_models(cfg)
...
def test_raises_on_pull_nonzero_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    network failure, etc.) → fail-fast with `OpendracoError` BEFORE the
    ...
    with pytest.raises(OpendracoError, match="exit code 1"):
        ollama_preflight.preflight_models(cfg)
...
def test_multi_model_pull_fail_fast_aborts_remaining(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    raises `OpendracoError` and never starts pulling subsequent models.
    ...
    with pytest.raises(OpendracoError, match="does-not-exist:0b"):
        ollama_preflight.preflight_models(cfg)
