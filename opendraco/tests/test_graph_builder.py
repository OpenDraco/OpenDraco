"""Tests for `opendraco.core.workflow.graph_builder` + the runner's
...
- cycles in the topology compile (runner caps execution via
  `recursion_limit = OPENDRACO_GRAPH_MAX_REVISITS * len(agents)`).
"""
...
from opendraco.exceptions.errors import TopologyError
...
def test_handoff_log_line_per_outgoing_edge(caplog: pytest.LogCaptureFixture) -> None:
    ...
    caplog.set_level("INFO", logger="opendraco.core.workflow.graph_builder")
    graph.invoke({})
...
def test_fan_in_with_shared_accumulator_does_not_crash() -> None:
    ...
    from opendraco.core.workflow.state_factory import build_state_class
...
def test_max_revisits_defaults_to_2(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from opendraco.core.workflow import runner

    monkeypatch.delenv("OPENDRACO_GRAPH_MAX_REVISITS", raising=False)
...
def test_max_revisits_reads_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("OPENDRACO_GRAPH_MAX_REVISITS", "5")
...
def test_max_revisits_invalid_falls_back(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("OPENDRACO_GRAPH_MAX_REVISITS", "not-a-number")
...
def test_max_revisits_clamps_to_one(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("OPENDRACO_GRAPH_MAX_REVISITS", "0")
    assert runner._max_revisits() == 1
    monkeypatch.setenv("OPENDRACO_GRAPH_MAX_REVISITS", "-3")
    assert runner._max_revisits() == 1
