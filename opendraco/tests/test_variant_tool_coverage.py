...
Each variant emitted by `opendraco.agents.types.variants.list_variants()` carries a
`default_tools` list -- the tool names the agent's whitelist references at
runtime. The MCP server (`opendraco.mcp.server.MCPServer`) needs to have a
...
Today's state:
* **Built-in OpenDraco variants** (`repo == "opendraco"`) reference tool names that
  are wired into `default_registry()` in `opendraco/mcp/server.py`. We hard-assert
...
* **Repo variants** (CSV-derived) reference upstream tool names whose
  ...
  `opendraco/tools/<repo>/`. Those stubs are NOT registered with MCP yet, so
  ...
  needed (run with `pytest -rx ./opendraco/tests/test_variant_tool_coverage.py`
  to see the message).
...
def test_builtin_variants_are_fully_covered_by_mcp_registry() -> None:
    """Hard floor: every built-in (`repo='opendraco'`) variant's `default_tools`
