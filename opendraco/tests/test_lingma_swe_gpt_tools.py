"""MCP registration + behavioral checks for `opendraco.tools.lingma_swe_gpt`.
...
`opendraco/tests/test_tools.py` (every tool is a LangChain `BaseTool`,
...
from opendraco.tools.repo.lingma_swe_gpt import (
    search_class,
...
# Lingma's `search_code` is the canonical OpenDraco BM25 search tool —
...
def test_tool_names_match_expected_inventory() -> None:
    ...
    `opendraco/config/agent_types/Lingma_SWE_GPT.json`."""
