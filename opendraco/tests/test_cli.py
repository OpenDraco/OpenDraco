"""Unit tests for the `opendraco` CLI surface (`opendraco/cli.py`).
...
in the `opendraco.cli` namespace and assert on the captured args. The CLI's
...
from opendraco import cli as cli_mod
...
def stub_runners(monkeypatch: pytest.MonkeyPatch) -> dict[str, list]:
    """Replace `_run_script` and `_run_shell_script` in `opendraco.cli` with
...
def stub_subprocess(monkeypatch: pytest.MonkeyPatch) -> list[tuple]:
    """Replace `subprocess.run` in the `opendraco.cli` namespace so the
...
def test_top_level_help_renders(runner: CliRunner) -> None:
    """`opendraco --help` must not raise and must list the registered
    sub-apps + commands."""
...
def test_ollama_pull_invokes_ollama_binary(
    ...
    """`opendraco ollama pull <model>` calls `ollama pull <model>`."""
...
def test_ollama_passthrough_forwards_args_after_double_dash(
    ...
    """`opendraco ollama -- rm qwen3:8b` should forward `rm qwen3:8b` to the
...
def test_ollama_passthrough_forwards_arbitrary_subcommand(
    ...
    `show` or `ps` (which opendraco doesn't wrap explicitly). Verifies
...
def test_run_prediction_forwards_required(
    ...
    assert result.exit_code == 0
    name, args = _last_forward(stub_runners)
    assert name == "generate_opendraco_predictions.py"
    assert args[:6] == [
        "--instances", "i.jsonl", "--output", "p.jsonl", "--config", "chain",
    ]
...
def test_run_evaluation_omits_subset_and_split(
    ...
    `opendraco run evaluation --predictions p.jsonl` must therefore succeed
...
def test_apply_forwards_optional_flags(
    ...
    assert "--model" in args and "opendraco-test" in args
...
def test_test_command_default(
    ...
    """`opendraco test` with no flags runs the full suite (no exclusions)."""
...
def stub_status_probes(monkeypatch: pytest.MonkeyPatch) -> None:
    """Neuter the two live-service probes so `opendraco status` never shells out
...
def test_status_reports_sections(
    ...
    """`opendraco status` prints the checklist and exits 0 on a healthy repo
...
def _extract_help_examples() -> list[str]:
    """Scan `opendraco/cli.py` for every `Example:  opendraco <body>` line and
    return the bodies (without the leading `opendraco `). The bodies live
    ...
    out: list[str] = []
    literal_re = re.compile(r'"((?:\\.|[^"\\])*Example:\s+opendraco\s+(?:\\.|[^"\\])+?)"')
    for m in literal_re.finditer(src):
        ...
        for line in literal.splitlines():
            ...
            if line.startswith("Example:"):
                ...
                if body.startswith("opendraco "):
                    body = body[len("opendraco "):].rstrip()
                if body:
                    out.append(body)
    # Triple-quoted docstring form: `Example:  opendraco …` followed by newline.
    ...
    for m in re.finditer(r"Example:\s+opendraco\s+([^\n]+)", src):
...
def test_help_example_parses_cleanly(
    ...
    """Every `Example: opendraco …` line in the help text must parse
    ...
    assert result.exit_code != 2, (
        f"`opendraco {example}` failed CLI parsing (exit 2):\n{out[:600]}"
    )
    assert "Missing option" not in out, (
        f"`opendraco {example}` claims a required option is missing:\n{out[:600]}"
    )
    assert "No such option" not in out, (
        f"`opendraco {example}` references an unknown option:\n{out[:600]}"
    )
