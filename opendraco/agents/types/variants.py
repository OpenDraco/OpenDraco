"""Agent-variant catalog merging built-in OpenDraco types with repo-derived variants under `opendraco/config/agent_types/*.json`; powers the Topology palette's per-type variant dropdown."""
...
from opendraco.agents.types import TYPE_COLORS, list_agent_types
...
def _builtin_variants() -> list[dict[str, Any]]:
    """One variant per built-in AGENT_TYPE; Topology dropdown shows the OpenDraco default first."""
    out: list[dict[str, Any]] = []
    for t in list_agent_types():
        agent_type = t["type"]
        out.append({
            "key":            f"opendraco:{agent_type}",
            "label":          f"OpenDraco · default",
            "repo":           "opendraco",
            "name":           agent_type,
...
def _repo_variants() -> list[dict[str, Any]]:
    """One variant per agent across every `opendraco/config/agent_types/*.json`."""
    out: list[dict[str, Any]] = []
...
def list_variants() -> dict[str, list[dict[str, Any]]]:
    """Variants grouped by canonical AGENT_TYPE; built-in OpenDraco variant is always first since the frontend treats the first entry as the default selection."""
