...
Lives at `opendraco/agents/router.py` (not under `types/`) because routing is a
...
`opendraco/agents/types/orchestrator.py`. It was renamed to `Router` because its
actual job is routing (one-line dispatch), not planning. The new
`PlannerOrchestrator` agent type under `opendraco/agents/types/` describes the
