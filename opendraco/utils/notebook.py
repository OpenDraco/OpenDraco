...
Lives in `opendraco.utils` (not `api/`) so non-FastAPI callers can produce
notebooks too.
"""
...
def resolve_evaluator(stem: str) -> tuple[str, bool]:
    ...
    `OPENDRACO_EVALUATOR.needs_wsl` flag (defaults False). Raises
    ...
    *,
    ...
    cells: list[dict[str, Any]] = [
        ...
        md(
            "## 1. Setup\n"
            "\n"
            "The notebook's `kernelspec.name = \"opendraco\"` (see metadata at "
            ...
            "\n"
            "If VSCode opens the notebook outside the OpenDraco workspace "
            ...
            "`.venv`/`venv` folders inside workspaces). `~/.opendraco-venv` is "
            ...
            "where the OpenDraco one lives — pick **\"Python 3 (OpenDraco)\"** "
            ...
            "confirm `opendraco` is registered (if not, re-run "
        ...
        code(
            ...
            "from pathlib import Path\n"
            "\n"
            "# Defensive sys.path prepend so `import opendraco...` still\n"
            "# resolves when the kernel isn't the opendraco-venv one.\n"
            "_venv = Path.home() / '.opendraco-venv'\n"
            ...
            "\n"
            "import opendraco.paths  # noqa: F401  # triggers load_dotenv(opendraco/.env)\n"
            "\n"
            "from opendraco.core.workflow.runner import run as run_opendraco\n"
            "from opendraco.utils.instances import fetch_swebench_instances\n"
            "\n"
        ...
        md(
            ...
            "- **`OPENDRACO_INSTANCES`** — override the SWE-bench instance "
        ...
        code(
            ...
            "for _k in ('OLLAMA_BASE_URL', 'SWEBENCH_API_KEY', 'OPENDRACO_INSTANCES',\n"
        ...
        md(
            "## 4. Inference\n"
            "\n"
            "Re-runs the OpenDraco workflow for each instance with the inlined "
        ...
        code(
            ...
            "        print(f'--- {iid} ---')\n"
            "        try:\n"
            "            patch = run_opendraco(inst, config=CONFIG)\n"
            ...
            "            'instance_id': iid,\n"
            "            'model_patch': patch,\n"
            "            'model_name_or_path': 'opendraco-notebook',\n"
            "        }\n"
        ...
        md(
            ...
            "at notebook-generation time (the `--evaluator` flag on `opendraco "
    ...
    return {
        "cells": cells,
        "metadata": {
            # `opendraco` kernelspec registered by `install.ps1` via `python
            # -m ipykernel install --user --name opendraco`. Falls back to
            # any Python 3 kernel via the setup cell's sys.path prepend.
            "kernelspec": {
                "name": "opendraco",
                "display_name": "Python 3 (OpenDraco)",
                "language": "python",
            },
            "language_info": {
                "name": "python",
