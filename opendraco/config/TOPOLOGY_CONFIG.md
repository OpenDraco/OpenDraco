...
Every OpenDraco run is driven by a single JSON file that defines the graph of
...
The loader resolves a config name as `predefined/<name>.json → loaded/<name>.json → <repo>/opendraco/config/<name>.json` (legacy flat root, kept for backward compat). First hit wins.
...
| `edges` | array | Directed edges. Each entry: `{ "from": "<src>", "to": "<dst>" }`. Cycles are allowed; the LangGraph runtime caps revisits via `OPENDRACO_GRAPH_MAX_REVISITS`. |
...
Map 1:1 to Pydantic `AgentConfig` (see `opendraco/config/loader.py:18`). Defaults: `think=true`, `num_ctx=4096`, `stream=true`, `temperature=0.2`, `top_k=40`, `top_p=0.9`, `min_p=0.0`, `repeat_penalty=1.1`, `repeat_last_n=64`, `seed=0`, `num_predict=-1`, `stop=[]`.
...
- `max_iters` — per-agent tool-loop cap. Once an agent has called `max_iters` tools without ending, the runner forces a final answer. Default depends on the agent class (see `opendraco/agents/types/<class>.py:DEFAULT_CONFIG`).
...
- `fallback` — available on any LLM-tool agent. `enabled=true` runs a single-shot patch attempt at end-of-run if the workspace has no `git diff`, so the agent at least produces some output instead of an empty patch. `guarantee_change=true` is a last-resort: if even the single-shot attempt produced no diff, the runner "touches" a tracked file (preferring `README.*`, otherwise the first `git ls-files` entry) so the harness gets a non-empty diff to apply. Implementation: `opendraco/agents/llm_tool_agent.py:57-60` and `:186-194`.
...
Loader code: `opendraco/config/loader.py:93` (variant injection) + `opendraco/agents/base_agent.py:70` (whitelist resolution).

## Variants

A `variant: "<RepoId>:<AgentName>"` looks up `opendraco/config/agent_types/<RepoId>.json` and copies that catalog entry's `prompts` and `tools` into the block — but **only** for fields the block hasn't already set inline. Used to faithfully mirror upstream multi-agent repos without forking every prompt into the predefined configs.
...
3. **Class names.** Use the AGENT_TYPE labels (`Locator`, `Patcher`, ...) — they're shorter and match what `opendraco/agents/types/__init__.py:TYPE_REGISTRY` recognizes.
...
6. **Inline `prompts`** for one-off overrides (Router dispatch logic, custom dispatcher messages). Otherwise rely on the class default in `opendraco/agents/types/<class>.py:DEFAULT_SYSTEM`.
7. **Wire edges deliberately.** Cycles are allowed and bounded by `OPENDRACO_GRAPH_MAX_REVISITS × num_agents` super-steps; for a fan-out hub topology, use a `Router` class on the hub so dispatch is conditional, not parallel.

## Quick reference

| Question | Look here |
|---|---|
| What model knobs are accepted? | `opendraco/config/loader.py` — `AgentConfig` Pydantic model |
| Which classes can I put in `class`? | `opendraco/agents/types/__init__.py` — `TYPE_REGISTRY` |
| What does each class's prompt + tool default look like? | `opendraco/agents/types/<class>.py` — `DEFAULT_SYSTEM` / `DEFAULT_USER` / `DEFAULT_TOOLS` |
| How do variants resolve? | `opendraco/config/loader.py:93` — `resolve_variant_block` |
| How are tools whitelisted at runtime? | `opendraco/agents/base_agent.py:70` |
| Which configs ship with OpenDraco? | `opendraco/config/predefined/` |
| How do I save my own? | `opendraco/config/loaded/` (via UI export or direct file write) |
