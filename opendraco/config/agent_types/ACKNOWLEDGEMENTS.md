...
OpenDraco ships a catalogue of agent "variants" sourced from 22 open-source
...
OpenDraco itself is the multi-agent orchestration framework; it does not claim
...
reproduction. OpenDraco's own redistribution will replace the verbatim
...
OpenDraco re-implements every callable tool from scratch in `opendraco/tools/`
rather than importing the upstream code, so the binary tool surface is
OpenDraco-authored. The acknowledgement here covers tool *design* and
*naming*, not implementation.

The tool-side counterpart of this file lives at
[`opendraco/tools/repo/ACKNOWLEDGEMENTS.md`](../../tools/repo/ACKNOWLEDGEMENTS.md);
it enumerates the per-repo subdirs under `opendraco/tools/repo/` and
