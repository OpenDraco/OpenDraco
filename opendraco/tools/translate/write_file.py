...
OpenDraco's existing repo-tools cover read / list / patch but never expose
...
`OPENDRACO_WORKSPACE_PATH` env var the runner sets before agents run;
...
def _active_workspace() -> Path | None:
    """The runner exports `OPENDRACO_WORKSPACE_PATH` before invoking the
    graph; the tool sandboxes writes against that value. Returns None
    when the env var is unset so callers can fail loud."""
    raw = (os.environ.get("OPENDRACO_WORKSPACE_PATH") or "").strip()
    if not raw:
        return None
    path = Path(raw)
...
def write_file(path: str, content: str) -> dict[str, Any]:
    ...
    if workspace is None:
        return {"ok": False, "error": "OPENDRACO_WORKSPACE_PATH is unset or not a directory"}
