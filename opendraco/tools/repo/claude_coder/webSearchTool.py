...
OpenDraco doesn't ship a web-search backend (no API key wiring, no
...
def webSearchTool(query: str) -> str:
    """Stub: no web-search backend configured in this OpenDraco install."""
    logger.warning("[claude_coder.webSearchTool] no backend: %s", query[:200])
    return (
        "error: webSearchTool is not configured in this OpenDraco install "
