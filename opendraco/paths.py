"""Canonical filesystem paths for the OpenDraco tree.
...
migrations of legacy folder names. The api server and `opendraco` CLI both
...
CONFIG_DIR: Path = BASE_DIR / "opendraco" / "config"
...
try:
    ...
    _load_dotenv(BASE_DIR / "opendraco" / ".env", override=False)
...
# Results root is overridable via RESULTS_DIR in opendraco/.env or
...
INFERENCE_INTERNAL_LOGS_DIR: Path = BASE_DIR / "opendraco" / "logs" / "inference_logs"
...
def bootstrap() -> None:
    ...
    1. Push BASE_DIR onto sys.path so `import api` / `import opendraco` work
       under the api server without an editable install hitting them.
    2. Load `opendraco/.env` (then `api/.env` for overrides).
    ...
    # Lazy import so a bare `import opendraco.paths` doesn't drag in dotenv.
    from dotenv import load_dotenv
    load_dotenv(BASE_DIR / "opendraco" / ".env", override=False)
