"""OpenDraco CLI — single `opendraco <subcommand>` entry covering instance
...
from opendraco.paths import BASE_DIR as REPO_ROOT, bootstrap
...
app = typer.Typer(
    ...
    help="OpenDraco command-line interface - drives instance generation, "
         "prediction, evaluation, notebook export, dev servers, and the "
         "test runner.",
    add_completion=False,
)
class _OllamaPassthroughGroup(TyperGroup):
    """Forwards unrecognised `opendraco ollama <args>` to the local
    `ollama` binary so `opendraco ollama -- rm qwen3:8b` works without
    wrapping every verb."""
...
ollama_app = typer.Typer(
    ...
    help="Local LLM lifecycle. Subcommands target the Ollama server named "
         "by `OLLAMA_BASE_URL` in opendraco/.env (default: localhost:11434)."
         "\n\nUse `opendraco ollama -- <args>` to forward arbitrary args to "
         "the `ollama` binary, e.g. `opendraco ollama -- rm qwen3:8b`.",
    add_completion=False,
)
run_app = typer.Typer(
    no_args_is_help=True,
...
def _script_needs_wsl(script_path: Path) -> bool:
    """True iff the script exposes `OPENDRACO_EVALUATOR = {"needs_wsl": True}`."""
    ...
    try:
        ...
        return False
    manifest = getattr(mod, "OPENDRACO_EVALUATOR", None)
...
def _run_script(script_name: str, extra_args: list[str]) -> int:
    ...
    `OPENDRACO_EVALUATOR.needs_wsl` get auto-wrapped in WSL on Windows."""
...
def ollama_pull(
    ...
    Example:  opendraco ollama pull qwen3.5:9b
    """
...
def ollama_serve(
    ...
    Example:  opendraco ollama serve --cpu-only
    """
    env = _ollama_host_env()
    if cpu_only:
...
@run_app.command(
    ...
    help=(
        ...
        "\n\nExample:  opendraco run instances --subset lite --split dev --output swebench_instances.jsonl --limit 5"
        "\nExample:  opendraco run instances --subset lite --split custom --output swebench_instances.jsonl --custom-repo owner/name --custom-problem \"calc returns difference\""
    ),
)
def run_instances(
    ctx: typer.Context,
...
@run_app.command(
    ...
    context_settings=_FORWARD_CTX,
    help=(
        "Generate OpenDraco predictions for the instances JSONL. Runs the "
        ...
        "\n\nExample:  opendraco run prediction --instances swebench_instances.jsonl --output opendraco_predictions.jsonl --config chain"
    ),
)
def run_prediction(
    ctx: typer.Context,
    instances: str = typer.Option(
        ..., "--instances",
        help="Path to the JSONL produced by `opendraco run instances`.",
    ),
    output: str = typer.Option(
        ..., "--output",
    ...
    config: str = typer.Option(
        ..., "--config",
        help=(
            "Config name (resolved against opendraco/config/predefined/ then "
    ...
    if limit is not None:
        forwarded += ["--limit", str(limit)]
    forwarded.extend(ctx.args)
    raise typer.Exit(_run_script("generate_opendraco_predictions.py", forwarded))
...
@run_app.command(
    ...
    help=(
        ...
        "\n  opendraco run evaluation --predictions opendraco_predictions.jsonl"
        ...
        "\nExample (remote):"
        "\n  opendraco run evaluation --remote --predictions opendraco_predictions.jsonl"
    ),
)
def run_evaluation(
    ctx: typer.Context,
    ...
    run_id: Optional[str] = typer.Option(
        None, "--run-id",
        help="Override the auto-generated `opendraco-<split>-<timestamp>` run id.",
    ),
...
@app.command(
    ...
    help=(
        ...
        "git, ollama, docker, node), the .env files, the ~/.opendraco-venv, the "
        "SWE-bench clone + its harness venv, and whether the Docker daemon "
        "and Ollama server are reachable."
        "\n\nExample:  opendraco status"
    ),
)
def status() -> None:
    typer.echo(typer.style("OpenDraco status", bold=True) + "\n")
    ...
    for name, why in (
        ...
        ("docker", "runs the SWE-bench harness for `opendraco run evaluation --local`"),
        ("npm", "builds/serves the Angular frontend (`opendraco web`)"),
    ):
    ...
    for rel in ("opendraco/.env", "api/.env"):
        path = REPO_ROOT / rel
        if path.is_file():
    ...
    venv_dir = Path.home() / ".opendraco-venv"
    venv_py = venv_dir / ("Scripts/python.exe" if os.name == "nt" else "bin/python")
    if venv_py.is_file():
        _print_check(_OK, "~/.opendraco-venv", str(venv_py))
    else:
        _print_check(
            _WARN, "~/.opendraco-venv",
...
@app.command(
    ...
    help=(
        ...
        "\n\nExample:  opendraco apply --predictions opendraco_predictions.jsonl --instances swebench_instances.jsonl --instance-id sqlfluff__sqlfluff-1625"
        "\nExample:  opendraco apply --predictions opendraco_predictions.jsonl --instances swebench_instances.jsonl --instance-id custom-<owner>-<name>-<short-sha> --report-dir results/evaluations"
    ),
)
def apply(
    ctx: typer.Context,
    predictions: str = typer.Option(
        ..., "--predictions",
        help="Path to the JSONL produced by `opendraco run prediction`.",
    ),
    instances: str = typer.Option(
        ..., "--instances",
        help="Path to the JSONL produced by `opendraco run instances`.",
...
@app.command(
    ...
    help=(
        ...
        "\n\nExample:  opendraco test --backend-only -- -k apply_description_fix"
        "\nExample:  opendraco test --frontend-only --integration -- --include \"src/integration/**\""
    ),
)
def test(
    ctx: typer.Context,
...
@app.command(
    ...
    help=(
        ...
        "\n\nExample:  opendraco notebook --predictions results/predictions/prediction-<run-id>.jsonl --evaluator apply_and_test"
    ),
)
def notebook(
    ...
    config: Optional[str] = typer.Option(
        ...
        help="Config name (stem) to bake into the notebook. Resolved against "
             "opendraco/config/predefined/ then loaded/. Mutually exclusive with --predictions.",
    ),
    instances: Optional[str] = typer.Option(
        None, "--instances",
        help="Path to a JSONL file whose lines carry `instance_id` (the same "
             "shape `opendraco run instances` writes). Each row's `instance_id` "
    ...
    from opendraco.utils.notebook import (
    ...
    if predictions:
        ...
        from opendraco.paths import LOADED_CONFIG_DIR, PREDEFINED_CONFIG_DIR
